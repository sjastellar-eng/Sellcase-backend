...
def normalize_olx_url(raw_url: str) -> str:
    """Normalize OLX URLs to a stable, parsable form.

    IMPORTANT:
    - We prefer the desktop domain (www.olx.ua). The mobile domain (m.olx.ua) often serves JS-heavy/minimal HTML
      and breaks simple HTML parsing.
    - Keep path/query as-is (except we strip fragments & whitespace).
    """
    ...
    # Normalize OLX UA domains to desktop
    if netloc.endswith("olx.ua"):
        netloc = "www.olx.ua"

    # Drop fragment; keep query (category, page, etc.)
